...
inline void PrintTo(StringRef const &Rf, std::ostream *OS) {
  std::format_to(std::ostream_iterator<char>(*OS), R"("{}")", Rf);
}
} // namespace clihehe::nao
...
TEST(StringRefTest, Iteration) {
  ...
  EXPECT_TRUE(equal(std::ranges::subrange(S.rbegin(), S.rend()), RefRev));
  EXPECT_TRUE(equal(std::ranges::subrange(CS.rbegin(), CS.rend()), RefRev));
}
...
TEST(StringRefTest, Misc) {
  std::string Storage;
  std::format_to(std::back_inserter(Storage), "{}", StringRef("hello"));
  EXPECT_EQ("hello", Storage);
}
...
//     const char *Str;
//     unsigned Expected;
//   } RadixNumbers[] = {{"123", 10}, {"1", 10}, {"0b1", 2}, {"01", 8}, {"0o1",
//   8},
//                       {"0x1", 16}, {"0", 10}, {"00", 8},  {"", 10}};
...
//     {"0x42-0x34", 66, "-0x34"},
//     {"0b101010", 42, ""},
//     {"0429F", 042, "9F"},            // Auto-sensed octal radix, invalid
//     digit
//     {"0x42G12", 0x42, "G12"},        // Auto-sensed hex radix, invalid digit
//     {"0b10101020101", 42, "20101"}}; // Auto-sensed binary radix, invalid
//     digit.
//
...
//   // First test empty strings.  We don't want these to allocate anything on
//   the
//   // allocator.
//   StringRef StrEmpty = "";
...
//   constexpr StringRef Cases[] = {"\rDoggo\rPupper", "Floofer\r",
...
TEST(StringRefTest, STLFormatting) {
  StringRef empty_sv("");
  EXPECT_EQ(std::format("{}", empty_sv), "");

  StringRef normal_sv("Hello World");
  EXPECT_EQ(std::format("{}", normal_sv), "Hello World");

  const char data[] = {'H', 'e', '\0', 'l', 'o'};
  StringRef sv(data, 5);

  std::string result = std::format("{}", sv);
  EXPECT_EQ(result.size(), 5);
  EXPECT_EQ(result[2], '\0');

  sv = "test";

  EXPECT_EQ(std::format("{:<10}", sv), "test      ");
  EXPECT_EQ(std::format("{:>10}", sv), "      test");
  EXPECT_EQ(std::format("{:^10}", sv), "   test   ");

  EXPECT_EQ(std::format("{:*<8}", sv), "test****");
  EXPECT_EQ(std::format("{:#^9}", sv), "##test###");

  sv = "Hello World";

  EXPECT_EQ(std::format("{:.5}", sv), "Hello");
  EXPECT_EQ(std::format("{:.0}", sv), "");
  ...
  EXPECT_EQ(std::format("{} and {}", sv1, sv2), "Apple and Banana");
  EXPECT_EQ(std::format("{1} then {0}", sv1, sv2), "Banana then Apple");

  sv = "Dynamic";
  /*volatile*/ int width = 12; // std::format doesn't work with `volatile int`
  /*volatile*/ int precision = 4;

  EXPECT_EQ(std::format("{:>{}}", sv, width), "     Dynamic");
  EXPECT_EQ(std::format("{:.{}}", sv, precision), "Dyna");
  EXPECT_EQ(std::format("{:>{}.{}}", sv, width, precision), "        Dyna");
}

#ifdef FMT_VERSION
TEST(StringRefTest, FMTFormatting) {
  StringRef empty_sv("");
  EXPECT_EQ(fmt::format("{}", empty_sv), "");

  StringRef normal_sv("Hello World");
  EXPECT_EQ(fmt::format("{}", normal_sv), "Hello World");
  ...
  EXPECT_EQ(fmt::format("{:<10}", sv), "test      ");
  EXPECT_EQ(fmt::format("{:>10}", sv), "      test");
  EXPECT_EQ(fmt::format("{:^10}", sv), "   test   ");

  EXPECT_EQ(fmt::format("{:*<8}", sv), "test****");
  EXPECT_EQ(fmt::format("{:#^9}", sv), "##test###");

  sv = "Hello World";

  EXPECT_EQ(fmt::format("{:.5}", sv), "Hello");
  EXPECT_EQ(fmt::format("{:.0}", sv), "");

  EXPECT_EQ(fmt::format("{:>10.5}", sv), "     Hello");

  constexpr StringRef utf8_sv("雀　すずめ🇨🇦㊗️");
  EXPECT_EQ(fmt::format("{}", utf8_sv), "雀　すずめ🇨🇦㊗️");

  StringRef sv1("Apple");
  StringRef sv2("Banana");

  EXPECT_EQ(fmt::format("{} and {}", sv1, sv2), "Apple and Banana");
  EXPECT_EQ(fmt::format("{1} then {0}", sv1, sv2), "Banana then Apple");
